...
const formatSeconds = (totalSeconds: number | 'blue') => {
	...
	private async tryFlash() {
		...
		const drives = availableDrives
			...
			.map((drive) => {
				return {
					...drive,
					statuses: constraints.getDriveStatuses(drive),
				};
			});
		...
		if (hasDangerStatus) {
			...
			this.setState({
				...
				drivesWithWarnings: drives.filter((driveWithWarnings) => {
					const hasStatuses = driveWithWarnings.statuses.length > 0;
					const systemXorLarge =
						!systemDrives &&
						driveWithWarnings.statuses.includes(constraints.statuses.large);
					return hasStatuses && (driveWithWarnings.isSystem || systemXorLarge);
				}),
				warningMessage: true,
			});
			return;
